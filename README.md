...
| [Agent contribution skill](.agents/skills/contribute-puregraph/SKILL.md) | Clone or fork, implement and check changes, open PRs, create issues, and comment. |
...
Give your agent the [contribution skill](.agents/skills/contribute-puregraph/SKILL.md) and describe the change, issue, or comment you want it to make. Codex can discover it in `.agents/skills/contribute-puregraph/`; with Claude Code or another tool, ask it to read that `SKILL.md` explicitly. For example:

> Read `.agents/skills/contribute-puregraph/SKILL.md`, implement [describe the change], run the relevant checks, and open a pull request to `puredesktop/puregraph` from my fork.
